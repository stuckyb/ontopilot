...
class ImportsBuildTarget(BuildTargetWithConfig):
    ...
    def getImportsIRIs(self):
        ...
        the import module build process).  For cases where the entire source
        ontology will be imported, the IRI of the source ontology is included.
        """
        mIRIs_list = []
        for row in self.tablerows:
            if row['abs_tfilepath'] == '':
                mIRIs_list.append(row['IRI'])
            else:
